        ...
        Surface(
            ...
            when (screen) {
                ...
                Screen.SETTINGS -> SettingsScreen(
                    ...
                    mood = mood,
                    voiceEnabled = voiceEnabled,
                    onNameChange = { name = it; saveAll() },
                    onCountryChange = { country = it; saveAll() },
                    onLanguageChange = { language = it; saveAll() },
                    onGenderChange = { gender = it; saveAll() },
                    onMoodChange = { mood = it; saveAll() },
                    onVoiceEnabledChange = { voiceEnabled = it; saveAll() },
                    onClearTasks = { tasks.clear(); saveAll() },
                    onBack = { screen = Screen.HOME }
                )
            }
        }
    ...
    Column(
        ...
        SimpleDropdown(
            ...
            options = listOf(
                ...
                "Australia", "Spain", "France", "Saudi Arabia", "Other"
            ),
        ...
        SimpleDropdown(
            label = "Language",
            selected = language,
            options = listOf("English", "Hindi", "Spanish", "French", "Arabic"),
        ...
        MoodButtons(selectedMood = mood, onMoodChange = onMoodChange)
...
fun HomeScreen(
    ...
    Column(
        ...
        Row(
            ...
            TextButton(onClick = goSettings) { Text("Settings") }
        }

        CardBox {
            Text(text = "Today's Focus", fontWeight = FontWeight.Bold, color = Charcoal)
            Spacer(modifier = Modifier.height(8.dp))
            val focus = activeTasks.firstOrNull()?.value?.title ?: "No task yet"
            Text(text = focus, color = Charcoal, style = MaterialTheme.typography.titleMedium)
            Spacer(modifier = Modifier.height(10.dp))
            Button(onClick = onMorningGreeting) { Text("Hear greeting") }
        }

        CardBox {
            Text(text = "Add a task", fontWeight = FontWeight.Bold, color = Charcoal)
            Spacer(modifier = Modifier.height(8.dp))
            OutlinedTextField(
                value = newTask,
            ...
            Spacer(modifier = Modifier.height(8.dp))
            Button(
                onClick = { onAddTask(newTask); newTask = "" },
                modifier = Modifier.fillMaxWidth()
            ) { Text("Add task") }
        }

        CardBox {
            Text(text = "Voice command (type)", fontWeight = FontWeight.Bold, color = Charcoal)
            Text(text = "Try: done · skip · later · add buy milk · what's next", color = MutedGray)
            Spacer(modifier = Modifier.height(8.dp))
            OutlinedTextField(
                value = commandText,
            ...
            Spacer(modifier = Modifier.height(8.dp))
            Button(
                onClick = { onVoiceCommand(commandText); commandText = "" },
                modifier = Modifier.fillMaxWidth()
            ) { Text("Run command") }
        }

        Text(text = "Today", fontWeight = FontWeight.Bold, color = Charcoal)
        ...
        if (completedTasks.isNotEmpty()) {
            Text(text = "Completed", fontWeight = FontWeight.Bold, color = Charcoal)
            completedTasks.forEach { item ->
                Text("✓ ${item.value.title}", color = CalmGreen)
            }
        }

        Button(onClick = goNight, modifier = Modifier.fillMaxWidth()) {
            Text("Night review")
        }
    }
}
...
fun NightScreen(
    ...
    Column(
        ...
        Text(
            text = "You completed $completed task(s). $active task(s) still open.",
            color = MutedGray
        )
        CardBox {
            Text(text = "Unfinished tasks", fontWeight = FontWeight.Bold, color = Charcoal)
            tasks.filter { it.status == "active" }.forEach {
                Text("○ ${it.title}", color = Charcoal)
            }
            if (active == 0) Text("Nothing waiting. Good reset.", color = MutedGray)
        }
        Button(onClick = onMoveAllActiveToTomorrow, modifier = Modifier.fillMaxWidth()) {
            Text("Keep unfinished tasks for tomorrow")
        }
        OutlinedButton(onClick = onDeleteSkipped, modifier = Modifier.fillMaxWidth()) {
...
fun SettingsScreen(
    ...
    Column(
        ...
        SimpleDropdown(
            label = "Country", selected = country,
            options = listOf("United States", "India", "United Kingdom", "Canada",
                "Australia", "Spain", "France", "Saudi Arabia", "Other"),
            onSelected = onCountryChange
        )
        SimpleDropdown(
            label = "Language", selected = language,
            options = listOf("English", "Hindi", "Spanish", "French", "Arabic"),
            onSelected = onLanguageChange
        )
        SimpleDropdown(
            label = "Gender", selected = gender,
            options = listOf("Female", "Male", "Non-binary", "Prefer not to say"),
            onSelected = onGenderChange
        )
        Text(text = "Mood selection", fontWeight = FontWeight.Bold, color = Charcoal)
        MoodButtons(selectedMood = mood, onMoodChange = onMoodChange)
        Row(
            modifier = Modifier.fillMaxWidth(),
            horizontalArrangement = Arrangement.SpaceBetween,
        ...
        Text(
            text = "No always-on mic. No cloud AI. No login. No ads.",
            color = MutedGray
        )
        OutlinedButton(onClick = onClearTasks, modifier = Modifier.fillMaxWidth()) {
...
@Composable
fun TaskCard(task: TaskItem, onDone: () -> Unit, onLater: () -> Unit, onSkip: () -> Unit) {
    CardBox {
        Text(text = task.title, fontWeight = FontWeight.Bold, color = Charcoal)
        Spacer(modifier = Modifier.height(10.dp))
        Row {
...
@Composable
fun MoodButtons(selectedMood: String, onMoodChange: (String) -> Unit) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                rowItems.forEach { mood ->
                    if (selectedMood == mood) {
                        Button(onClick = { onMoodChange(mood) }) { Text(mood) }
                    } else {
                        OutlinedButton(onClick = { onMoodChange(mood) }) { Text(mood) }
                    }
                }
            }
        }
...
fun SimpleDropdown(
    ...
    options: List<String>, onSelected: (String) -> Unit
) {
...
fun handleSimpleCommand(
    ...
    if (lower.isBlank()) { say("I missed that."); return }

    when {
        lower.startsWith("add ") -> {
            val title = command.removePrefix("add").trim()
            if (title.isNotBlank()) { tasks.add(TaskItem(title)); save(); say("Added.") }
            else say("What task should I add?")
        }
        lower.startsWith("remind me to ") -> {
            val title = command.removePrefix("remind me to").trim()
            if (title.isNotBlank()) { tasks.add(TaskItem(title)); save(); say("Added.") }
            else say("What should I remind you to do?")
        }
        lower.contains("done") || lower.contains("complete") || lower.contains("finished") -> {
            val index = tasks.indexOfFirst { it.status == "active" }
            if (index >= 0) { tasks[index] = tasks[index].copy(status = "completed"); save(); say("Done.") }
            else say("No active task to complete.")
        }
        lower.contains("skip") -> {
            val index = tasks.indexOfFirst { it.status == "active" }
            if (index >= 0) { tasks[index] = tasks[index].copy(status = "skipped"); save(); say("Skipped.") }
            else say("No active task to skip.")
        }
        lower.contains("later") -> { save(); say("Okay. I'll keep it for later.") }
        lower.contains("tomorrow") -> { save(); say("Saved for tomorrow.") }
        lower.contains("what") && lower.contains("next") -> {
            val next = tasks.firstOrNull { it.status == "active" }
            if (next != null) say("Next is ${next.title}.") else say("Nothing active right now.")
        }
        lower.contains("night") || lower.contains("plan tomorrow") -> {
            say("Let's clear your list for tomorrow.")
        }
        else -> say("Try: add, done, later, skip, or what's next.")
    }
}
